...
def piotroski_score(df):
    ...
    output_frame['p_score_3'] = np.where(output_frame.groupby('isin')['return_on_assets_%'].shift(0) >
                                         output_frame.groupby('isin')['return_on_assets_%'].shift(1), 1, 0)
    output_frame['p_score_4'] = np.where(output_frame['operating_cash_flow_mil'] > output_frame['net_income_mil'], 1, 0)
    output_frame['p_score_5'] = np.where(output_frame.groupby('isin')['long-term_debt'].shift(0) <
                                         output_frame.groupby('isin')['long-term_debt'].shift(1), 1, 0)
    output_frame['p_score_6'] = np.where(output_frame.groupby('isin')['current_ratio'].shift(0) >
                                         output_frame.groupby('isin')['current_ratio'].shift(1), 1, 0)
    output_frame['p_score_7'] = np.where(output_frame.groupby('isin')['shares_mil'].shift(0) <=
                                         output_frame.groupby('isin')['shares_mil'].shift(1), 1, 0)
    output_frame['p_score_8'] = np.where(output_frame.groupby('isin')['gross_margin_%'].shift(0) >
                                         output_frame.groupby('isin')['gross_margin_%'].shift(1), 1, 0)
    output_frame['p_score_9'] = np.where(output_frame.groupby('isin')['asset_turnover'].shift(0) >
                                         output_frame.groupby('isin')['asset_turnover'].shift(1), 1, 0)
    output_frame['p_score'] = np.sum(output_frame[['p_score_1', 'p_score_2', 'p_score_3', 'p_score_4',
                                                   'p_score_5', 'p_score_6', 'p_score_7', 'p_score_8', 'p_score_9']], axis=1)
...
def magic_formula(df):
    output_frame = df.copy()
    output_frame['magic_formula_score'] = ((1/output_frame['ev_ebitda_ratio']).clip(0, None) *
                                           output_frame['return_on_invested_capital_%'].clip(0, None)).replace(np.inf, 0)
    output_frame['rank_magic_formula'] = output_frame[['magic_formula_score']].rank(
        method='dense', ascending=False).replace(np.nan, df['isin'].nunique()+1).values
    return output_frame
...
def oshaugnessy_value_composite(df):
    ...
    output_frame['shareholder_yield'] = (((output_frame.groupby('isin')['shares_mil'].shift(1)-output_frame.groupby('isin')['shares_mil'].shift(
        0))/output_frame.groupby('isin')['shares_mil'].shift(1)) + (output_frame['dividends'] / output_frame['currentprice'])).fillna(0)
    output_frame['oshaugnessy_score'] = (output_frame['ev_ebitda_ratio'].rank(method='dense', ascending=True) + output_frame['pricetobook'].rank(method='dense', ascending=True) + output_frame['trailingpe'].rank(method='dense', ascending=True) +
                                         output_frame['price_to_sales'].rank(method='dense', ascending=True) + output_frame['price_to_cash_flow'].rank(method='dense', ascending=True) + output_frame['shareholder_yield'].rank(method='dense', ascending=False))
    output_frame['rank_oshaugnessy'] = output_frame[['oshaugnessy_score']].rank(
        method='dense', ascending=True).replace(np.nan, df['isin'].nunique()+1).values
    return output_frame
...
def roic_croic(df):
    ...
    output_frame = df.copy()
    output_frame['croic'] = (output_frame['free_cash_flow_mil'] / (output_frame['net_income_mil'] /
                                                                   (output_frame['return_on_invested_capital_%']/100))).fillna(np.inf)
    ...
    output_frame['croic_3yr_inc'] = np.where((output_frame['croic'].values > output_frame['croic_1yr_lag'].values) * (
        output_frame['croic_1yr_lag'].values > output_frame['croic_2yr_lag'].values) * (output_frame['croic_2yr_lag'].values > output_frame['croic_3yr_lag'].values), 1., 0)
    ...
    output_frame['roic_3yr_inc'] = np.where((output_frame['return_on_invested_capital_%'].values > output_frame['roic_1yr_lag'].values) * (
        output_frame['roic_1yr_lag'].values > output_frame['roic_2yr_lag'].values) * (output_frame['roic_2yr_lag'].values > output_frame['roic_3yr_lag'].values), 1., 0)
...
def get_valuation_ratios(yahoo_tickers):
    ...
    nr_failed = 0
    out_cols = ['targetmedianprice', 'numberofanalystopinions', 'ebitda', 'currentprice',
                'pricetobook', 'marketcap', 'trailingpe', 'forwardpe', 'recommendationkey']
    ...
    out_frame['nnwc'] = (out_frame['cash'] + 0.75*out_frame['netreceivables'] + 0.5*out_frame['inventory'] -
                         out_frame['totalliab'] - out_frame['marketcap'])/out_frame['marketcap']
...
def stock_screener():
    ...
    if os.path.exists(os.path.join(os.getcwd(), 'stock_data', 'listed_companies.csv')):
        listed_companies_df = pd.read_csv(os.path.join(os.getcwd(), 'stock_data', 'listed_companies.csv'),
                                          index_col=0, low_memory=False).drop_duplicates()
    else:
    ...
    valuation_path = os.path.join(os.getcwd(), 'stock_data', 'valuation_ratios_{}.csv'.format(
    ...
    screened_stocks_output = screened_stocks.copy()[['name', 'isin', 'yahoo_ticker', 'sector', 'currency', 'marketcap_sci', 'recommendationkey', 'currentprice', 'targetmedianprice', 'numberofanalystopinions', 'forwardpe', 'trailingpe', 'ev_ebitda_ratio', 'pricetobook', 'p_score',
                                                     'return_on_invested_capital_%', 'roic_1yr_lag', 'roic_2yr_lag', 'roic_3yr_lag', 'croic', 'croic_1yr_lag', 'croic_2yr_lag', 'croic_3yr_lag', 'ncav', 'nnwc', 'rank_piotroski', 'rank_magic_formula', 'rank_oshaugnessy', 'rank_croic', 'rank_roic', 'rank_ncav', 'rank_nnwc']]
    screened_stocks_output.loc[:, 'combined_rank'] = (screened_stocks_output['rank_roic'] + screened_stocks_output['rank_ncav'] + screened_stocks_output['rank_nnwc'] + screened_stocks_output['rank_croic'] +
                                                      screened_stocks_output['rank_piotroski']+screened_stocks_output['rank_magic_formula']+screened_stocks_output['rank_oshaugnessy']).rank(method='dense', ascending=True).replace(np.nan, screened_stocks_output.shape[0]+1).values
